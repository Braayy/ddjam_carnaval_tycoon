...
func draw():
	...
	var path: Path3D = line3D	# following these
...
#		return
	...
	surface_tool.set_smooth_group(smooth)
	...
	for i in range(steps + 1):
		...
		if line3D.width_curve:
			taper_size = line3D.width_curve.sample_baked(float(i) / float(steps))
		var node = Node3D.new()
		...
		for j in range(count):
			...
			if line3D.gradient != null:
				...
				surface_tool.set_color(line3D.default_color)
			else:
				surface_tool.set_color(Color(1, 1, 1, 1))
